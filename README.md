...
- Local Drive 直传模式（绕过 CDN）
  - 前端直接上传文件到 Local Drive 服务器，不再经过 ImgBed 服务器中转
  - 上传完成后仅提交元数据到 ImgBed 后端记录，请求体极小
  - 彻底解决 CDN 大文件上传 524 超时问题
  - 直传配置从 ImgBed 后端获取（serverUrl、authToken、agentId），前端缓存避免重复请求
